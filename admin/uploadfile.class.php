...
class uploadfile
{
	...
	public $renamed = true;//是否重新命名上传文件
	...
	*/
	function _set_options($arrFile)
	{
		if (is_array($arrFile))
		{
			...
			{
				if (in_array($key, array("path", "allow_type", "max_size", "overwrite", "renamed", "error_mode", "filename", "ifresize", "resize_width", "resize_height", "cut", "attachmentspath", "uploadsavetype")))
				{
					$this->$key = $value;
				}
			}
	...
	*/
	function _check_size()
	{
		...
		{
			$this->error("_check_size()：".$this->upfile['name']."文件大小超过了限制".$this->max_size."MB");
		}
	}
